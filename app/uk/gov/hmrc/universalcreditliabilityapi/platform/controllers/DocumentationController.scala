...
class DocumentationController @Inject() (assets: Assets, cc: ControllerComponents, appConfig: AppConfig)
  ...
  def definition(): Action[AnyContent] = Action {
    val definitionBase = Json.parse(Source.fromResource("api.definition.defaults/base.json").mkString(""))
    val versions       = Json
      .parse(Source.fromResource("api.definition.defaults/versions.json").mkString(""))
      ...
      __.read[JsArray]
        .map { case JsArray(array) =>
          JsArray(array.map { obj =>
            ...
            val endpointsEnabled     = maybeConfigOverrides.flatMap(_.endpointsEnabled).orElse(defaultEndpointsEnabled)
